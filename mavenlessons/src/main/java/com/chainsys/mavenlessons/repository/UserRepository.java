...
//	Passport findByPassportId(int id);
	...
	@Query("select * from User u where u.id=?1")	// ?1, ?2, ?3 => Positioned parameters
	User getUserNativeSQL1(int uid); // This is example of JPQL
	
	@Query(value="select * from User u where u.id=?1",nativeQuery=true)
	User getUserNativeSQL(int uid); // This is example of NativeQuery
	// By default @Query will execute only select statements
    // To execute insert,update,delete, statement @Query should be marked as @modifying
	@Modifying
